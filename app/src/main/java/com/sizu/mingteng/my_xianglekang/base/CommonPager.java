...
import com.sizu.mingteng.my_xianglekang.util.UIUtils;
...
public abstract class CommonPager {
    // 问题：
    // 1、在onResume方法处理流程的开始，但是Fragment的onResume方法是与Activity绑定，
    // 对于其他Fragment我们如果使用onResume方法作为流程的开始，就会出现提前加载数据的问题。
     ...
     */
    public void dynamic() {
        if (onLoadingData || isReadData) //判断是否调用 加载数据  或者 已读取数据
            return;
        onLoadingData = true;
        showProgress();
        loadData();
    ...
    public void showProgress() {
        loading = View.inflate(App.getContext(), R.layout.pager_loading, null);
    ...
    public void showEmpty() {
        empty = View.inflate(App.getContext(), R.layout.pager_empty, null);
        changeViewTo(empty);
    }
